    ...
    <div class="max-w-lg w-full bg-white shadow-sm rounded-2xl p-8 text-center">
      ...
      <template v-else-if="paymentInfo && !success && !alreadyConfirmed">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 mb-4">
          Confirm Rent Due Date Change
        </h1>
        <p class="text-sm text-gray-600 mb-6">
          Please confirm you have made the payment for your rent due date change:
        </p>
        ...
        <div class="bg-gray-50 rounded-xl p-6 mb-6 text-left">
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span class="text-sm font-medium text-gray-900 text-right">{{ paymentInfo.propertyAddress }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500">Current Due Day</span>
              <span class="text-sm font-medium text-gray-900">{{ paymentInfo.currentDueDay }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500">New Due Day</span>
              <span class="text-sm font-medium text-gray-900">{{ paymentInfo.newDueDay }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500">Effective From</span>
              <span class="text-sm font-medium text-gray-900">{{ paymentInfo.effectiveDate }}</span>
            </div>
            <div class="border-t border-gray-200 my-2"></div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500">Pro-rata Amount</span>
              <span class="text-sm font-medium text-gray-900">£{{ paymentInfo.proRataAmount.toFixed(2) }}</span>
            </div>
            <div v-if="paymentInfo.adminFee > 0" class="flex justify-between">
              <span class="text-sm text-gray-500">Admin Fee</span>
              <span class="text-sm font-medium text-gray-900">£{{ paymentInfo.adminFee.toFixed(2) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500">Total Amount Due</span>
      ...
      <template v-else-if="success">
        ...
        <p class="text-sm text-gray-600 mb-4">
          Thank you for confirming your rent due date change payment.
        </p>
        <p class="text-sm text-gray-600 mb-4">
          The agent has been notified and will verify the payment has been received. Once confirmed, your new rent due date will take effect.
        </p>
      ...
      <template v-else-if="alreadyConfirmed">
        ...
        <p class="text-sm text-gray-600 mb-4">
          You've already confirmed your payment for this rent due date change.
        </p>
        <p class="text-sm text-gray-600">
          The agent is processing your request. If you have any questions, please contact them directly.
        </p>
      </template>
...
async function fetchPaymentInfo() {
  ...
  try {
    const response = await fetch(`${API_URL}/api/tenancies/rent-due-date-change/${token}`)
...
async function confirmPayment() {
  ...
  try {
    const response = await fetch(`${API_URL}/api/tenancies/rent-due-date-change/${token}/confirm-payment`, {
      method: 'POST',
      headers: {
