                  ...
                  <div v-for="(segment, segmentName) in displaySegments" :key="segmentName" class="bg-white rounded-lg border border-gray-200">
                    ...
                    >
                      ...
                      <svg
                        ...
                        :class="{ 'rotate-180': expandedSegments.includes(segmentName) }"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div v-if="expandedSegments.includes(segmentName)" class="border-t border-gray-200">
...
const toggleSegment = (segmentName: string) => {
  const index = expandedSegments.value.indexOf(segmentName)
  if (index > -1) {
    expandedSegments.value.splice(index, 1)
  } else {
    expandedSegments.value.push(segmentName)
  }
}
...
const getSegmentEmoji = (segmentName: string) => {
  ...
  return emojis[segmentName] || '📊'
}

const getSegmentTitle = (segmentName: string) => {
  ...
  return titles[segmentName] || segmentName
}
