  ...
  <Sidebar>
    <div class="p-8">
      ...
      <div class="mt-12 bg-white rounded-lg shadow p-6">
        <div class="mb-6">
          <h3 class="text-2xl font-bold text-gray-900">Agreement History</h3>
...
const filteredAgreements = computed(() => {
  ...
  return agreements.value.filter((agreement: any) => {
    const propertyAddress = formatPropertyAddress(agreement.property_address).toLowerCase()
    const tenantNames = getTenantNames(agreement.tenants).toLowerCase()
    return propertyAddress.includes(query) || tenantNames.includes(query)
  })
})
...
// Helper functions for agreement history
function formatPropertyAddress(address: any): string {
  if (!address) return 'N/A'
  return `${address.line1}, ${address.city}, ${address.postcode}`
}

function getTenantNames(tenants: any[]): string {
  if (!tenants || tenants.length === 0) return 'N/A'
  return tenants.map((t: any) => t.name).join(', ')
}
...
function downloadAgreement(pdfUrl: string | null) {
  if (!pdfUrl) {
    error.value = 'PDF not available for this agreement'
    return
  }
  window.open(pdfUrl, '_blank')
}

// Fetch all agreements for the company
async function fetchAgreements() {
  ...
  try {
    ...
    if (response.ok) {
      const { agreements: data } = await response.json()
