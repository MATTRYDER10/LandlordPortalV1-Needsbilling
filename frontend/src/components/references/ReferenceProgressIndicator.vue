...
const props = defineProps<{
  completed: boolean
  label: string
  icon: 'user' | 'wallet' | 'home' | 'credit-card' | 'shield'
}>()
...
const indicatorClasses = computed(() => {
  ...
  return 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600'
})
</script>
