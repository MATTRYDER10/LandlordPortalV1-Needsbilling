...
/**
 * POST /api/tenancies/records/:id/deposit-payout
 * Mark deposit as paid out to landlord
 */
router.post('/records/:id/deposit-payout', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const companyId = await getCompanyIdForRequest(req)
    const userId = req.user?.id
    if (!companyId) {
    ...
    const { data: tenancy, error: tenancyError } = await supabase
      .from('tenancies')
      .select('id, deposit_amount, deposit_protected_at, deposit_paid_out_at')
      .eq('id', tenancyId)
      .eq('company_id', companyId)
      .is('deleted_at', null)
      .single()
    ...
    if (tenancy.deposit_paid_out_at) {
      return res.status(400).json({ error: 'Deposit has already been marked as paid out' })
    }

    // Update tenancy with payout info
    const { error: updateError } = await supabase
      .from('tenancies')
      .update({
        deposit_paid_out_at: new Date().toISOString(),
        deposit_paid_out_amount: amount || tenancy.deposit_amount,
        deposit_paid_out_notes: notes || null
      })
      .eq('id', tenancyId)

    if (updateError) {
      console.error('Error marking deposit as paid out:', updateError)
      return res.status(500).json({ error: 'Failed to update deposit payout status' })
    }

    // Log activity
    await tenancyService.logTenancyActivity({
      tenancyId,
      type: 'deposit_paid_out',
      category: 'financial',
      summary: `Deposit of £${amount || tenancy.deposit_amount} marked as paid out to landlord`,
      metadata: {
        amount: amount || tenancy.deposit_amount,
        notes: notes || null
      },
      performedBy: userId || undefined
    })
