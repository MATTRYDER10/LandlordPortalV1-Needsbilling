...
router.post('/:token/submit', async (req: Request, res: Response) => {
  try {
    ...
    const updateData: any = {
      ...
      // Referee details — populate encrypted columns for direct lookups
      employer_ref_email_encrypted: income.employerRefEmail ? encrypt(income.employerRefEmail) : null,
      employer_ref_name_encrypted: income.employerRefName ? encrypt(income.employerRefName) : null,
      employer_ref_phone_encrypted: income.employerRefPhone ? encrypt(income.employerRefPhone) : null,
      previous_landlord_email_encrypted: residential.currentLandlordEmail ? encrypt(residential.currentLandlordEmail) : null,
      previous_landlord_name_encrypted: residential.currentLandlordName ? encrypt(residential.currentLandlordName) : null,
      previous_landlord_phone_encrypted: residential.currentLandlordPhone ? encrypt(residential.currentLandlordPhone) : null,
      accountant_email_encrypted: income.accountantEmail ? encrypt(income.accountantEmail) : null,
      accountant_name_encrypted: income.accountantName ? encrypt(income.accountantName) : null,
