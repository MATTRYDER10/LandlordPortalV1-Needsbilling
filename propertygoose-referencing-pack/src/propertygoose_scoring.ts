...
export function scorePropertyGoose(a: ApplicantInput): ScoredDecision {
  const decline = (reason: string): ScoredDecision => ({
    decision: "DECLINE",
    score_total: 0, domain_scores: { credit_tas: 0, affordability: 0, employment: 0, residential: 0, id_data: 0 },
    ratio: 0, caps: [], review_flags: [], decline_reasons: [reason],
    guarantor_requirements: { min_ratio: 3.0, min_tas: 618, min_tas_if_rent_protection: 710 }
  });

  // Gates
  if (a.aml_pep_sanctions.sanctions_match) return decline("Sanctions match");
  if (a.aml_pep_sanctions.aml_adverse) return decline("Adverse AML");
  if (!a.id_check.passed || a.id_check.document_fraud) return decline("ID failed");
  if (a.credit_events.undeclared_adverse_found) return decline("Undeclared adverse credit");
  ...
  if (ratio < 1.5) return decline("Affordability < 1.5x");
  ...
  // D) Residential
  let res_pts = 0;
  if (!a.landlord_ref.received) { res_pts = 6; review_flags.push("missing_landlord_ref"); }
  else {
    if (a.landlord_ref.arrears === "severe" || a.landlord_ref.property_care === "bad") return decline("Severe tenancy issues");
    else if (a.landlord_ref.arrears === "minor_cleared") res_pts = 8;
    else res_pts = 15;
  }
  ...
  if (review_flags.length > 0) {
    return {
      ...
      ratio, caps, review_flags, decline_reasons: declineReasons,
      guarantor_requirements: { min_ratio: 3.0, min_tas: 618, min_tas_if_rent_protection: 710 }
    };
  }
